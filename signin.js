...
async function loadVisitStats() {
  ...
  let d1 = "—", d7 = "—", d30 = "—", note = "", noteIsTrouble = false;
  try {
    const snap = await getDocs(collection(db, "stats", "visits", "daily"));
    const byDay = {};
    snap.forEach((d) => { byDay[d.id] = d.data().count || 0; });
    if (Object.keys(byDay).length === 0) {
      // Читать дают, а записей нет ни одной. Причин РОВНО ДВЕ, и отличить их
      // отсюда нечем: либо после включения посуточного счёта ещё никто не
      // заходил, либо правило пускает read, но не create/update. Называем обе
      // и не выдаём догадку за факт. Ноль тут показывать нельзя в любом случае.
      noteIsTrouble = true;
      note = "Посуточных записей нет ни одной. Либо после включения счёта ещё "
           + "никто не заходил, либо правило Firestore пускает чтение, но не "
           + "запись. Отличить отсюда нечем. Общий счёт при этом идёт.";
    } else {
      const sum = (n) => {
        let s = 0;
      ...
      // Без этой строки «всего 35, за 30 дней 0» читается как «за месяц никто
      // не заходил». Правда другая: посуточный счёт завели позже общего, и за
      // дни до него чисел нет и быть не может. Дату берём из самой ранней
      // записи, а не хардкодим — она не соврёт при переносе.
      const since = Object.keys(byDay).sort()[0];
      note = "Посуточный счёт ведётся с " + since + ". За дни раньше этой даты "
           + "чисел нет — общий счёт при этом накоплен за всё время.";
    }
  } catch (e) {
    noteIsTrouble = true;
  ...
  box.innerHTML = `
    ...
    <p class="text-xs text-gray-500">
      ...
      ${note ? `<br><span class="${noteIsTrouble ? "text-orange-600" : ""}">${escapeHtml(note)}</span>` : ""}
    </p>`;
}
